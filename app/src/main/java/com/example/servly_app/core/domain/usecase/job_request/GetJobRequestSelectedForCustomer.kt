...
class GetJobRequestSelectedForCustomer(private val jobRequestRepository: JobRequestRepository) {
    suspend operator fun invoke(jobPostingId: Long): Result<JobRequestInfo?> {
        return jobRequestRepository.getJobRequestSelectedForCustomer(jobPostingId)
    }
}